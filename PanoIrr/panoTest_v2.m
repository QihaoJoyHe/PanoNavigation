...
function [testResult, esc] = panoTest_v2(w, xc, yc, testResult, panoNum, align, Seriesnum, i, esc)
...
baseRect = circleRectOuter;
trueAngle = testFrameIdx - 1;

% responseAngle
Screen('FillArc', w, red, baseRect, responseAngle - fov/2, fov);  
Screen('FillArc', w, black, circleRectInner, responseAngle - fov/2, fov);  

% trueAngle
Screen('FillArc', w, [0 0 255], baseRect, trueAngle - fov/2, fov);  
Screen('FillArc', w, black, circleRectInner, trueAngle - fov/2, fov);  
...
if i < trialnum
    ...
    DrawFormattedText(w, ['Difference: ', num2str(angleDiff)], 'center', yc, white); 
end
Screen('Flip', w);
WaitSecs(feedback);
