% Pano exp, pilot code
% Phase1: learning, free exploration, 45s timeLimit, record keyPress
% Phase2: test, record responseAngle, give feedback
% 20 panos for each participant, 30min
...
ResultSavingPath = [currPath,'/ResultsSummary/',num2str(Seriesnum),'_PanoResults'];
...
load('CrossPanoList.mat', 'CrossPano');
% Load practice
load('CrossPracList.mat', 'CrossPrac');
...
try
    ...
    for prac = 1:pracnum
        ...
        [pracPressData, pracResult, esc] = panoPrac(w, xc, yc, prac, align, pracPressData, pracResult, Seriesnum, esc);
    ...
    for i = 1:trialnum
        % Align
        align = randi([-44, 45]);  % [-44, 45], randomly selected
        ...
        % Test phase
        [testResult, esc] = panoTest(w, xc, yc, testResult, test_seq(i), align, Seriesnum, i, esc);
